// GET /api/facturas?modo=descargar&pack_id=X&doc_id=Y
//   -> Descarga el PDF de la factura que subió el vendedor
// GET /api/facturas?modo=revisar&offset=0&limit=30
//   -> Revisa pedidos sin factura y les pregunta a ML si ya la subieron
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (modo === 'descargar') {
      ...
      const resp = await fetch(`https://api.mercadolibre.com/packs/${pack_id}/fiscal_documents/${doc_id}`, { headers: { Authorization: `Bearer ${accessToken}` } });
      if (!resp.ok) {
    ...
    if (modo === 'revisar') {
      ...
      for (const p of pendientes || []) {
        ...
        if (factura) {
          await supabase.from('pedidos').update({
            factura_ml_id: factura.id, factura_ml_filename: factura.filename, factura_ml_fecha: factura.fecha,
      ...
      return res.status(200).json({ ok: true, procesados: pendientes?.length || 0, encontradas, total, next_offset: offset + limit, done });
    }

    return res.status(400).json({ error: 'Falta el parámetro modo (descargar o revisar)' });
  } catch (err) {
